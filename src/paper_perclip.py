...
Target: the PAPER's Table VII mean of 96.06% -- not the notebook's 94.29%, which never reproduced
...
for fold, (tr, va) in enumerate(skf.split(pairs, LABELS), 1):
    ...
    print(f"  fold {fold} fusion best val acc: {best_acc:.2f}%   (paper fold target ~94-96%)",
          flush=True)
    pd.DataFrame(rows).to_csv(os.path.join(OUT, "paper_perclip_folds.csv"), index=False)

df = pd.DataFrame(rows)
print("\n\n===== PAPER PROTOCOL + per-clip norm (as published) + StandardScaler =====")
...
df.to_csv(os.path.join(OUT, "paper_perclip_folds.csv"), index=False)
