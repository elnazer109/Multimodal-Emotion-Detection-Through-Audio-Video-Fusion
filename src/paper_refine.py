...
ONE thing changes from the original: StandardScaler on the 568-dim fused vector. Raw MFCC means
sit near -300 next to 0..1 softmax probabilities, and FusionMLP's first BatchNorm is *after* the
first Linear, so nothing ever normalized the input.

Video normalization is the ORIGINAL per-clip standardization, deliberately. A sibling run
(ravdess-paper-protocol) swapped it for Kinetics channel statistics on the theory that feeding
Kinetics-pretrained weights per-clip-standardized inputs "fights" them. That theory was wrong:

    paper Table VII video mean : 92.72
    original notebook, fold 1  : 91.24   (per-clip standardization)
    Kinetics run, fold 1       : 85.54   (-5.7)
    Kinetics run, fold 3       : 71.22   (collapse)

Per-clip standardization removes per-clip lighting/contrast variation, and RAVDESS records each
actor under consistent studio conditions, so killing that nuisance variation evidently beats
matching Kinetics' global stats here. This run keeps the scaler (which helps) and drops the
normalization change (which hurt).
...
The paper (Sec. VII.2) describes the video pipeline as r3d_18 -> "additional 3D convolutional
layers are stacked, followed by global average pooling for dimension reduction", and Fig. 4 shows
Conv3D(256,128,64) doing that reduction. The notebook slices the backbone at children()[:-1],
which KEEPS r3d_18's own AdaptiveAvgPool3d -- so the volume is already collapsed to 1x1x1 before
the 3x3x3 convs run. Every one of them then convolves 26/27 zero-padding and degenerates to its
centre tap: a stack of Linear layers wearing Conv3d costumes, and the paper's stated architecture
is simply not implemented.

Slicing at [:-2] instead keeps stem..layer4, so the backbone emits (B,512,2,7,7) for a 16x112x112
clip and the refine convs do the spatiotemporal work the paper describes. emb512 (the 512-d
fusion feature) is still r3d_18's pooled embedding, unchanged -- pooled explicitly here rather
than by the sliced-off layer.

Cheap: Conv3d(512,256,k=3) over a 2x7x7 volume is ~0.35 GMAC/sample against r3d_18's own ~40
GFLOP, so this costs almost nothing on top.
...
# Per-clip standardization, verbatim from the original notebook's extract_video_frames:
#     mu, sigma = frames.mean(), frames.std() + 1e-6
#     frames = (frames - mu) / sigma
# A single SCALAR mean/std over the whole (T,H,W,C) clip -- not per-channel. Scale-invariant, so
# it does not matter that the cache stores uint8 rather than the original's float32.
...
class VideoModel(nn.Module):
    def __init__(self, n_classes=8, dropout=0.4):
        ...
        # [:-2] drops r3d_18's own avgpool AND fc -> emits (B,512,2,7,7), a real volume.
        # The notebook's [:-1] keeps avgpool, collapsing to 1x1x1 before refine ever runs.
    ...
    def forward(self, x):
        ...
        emb512 = F.adaptive_avg_pool3d(feat, 1).flatten(1)        # (B,512) -- unchanged fusion feature
        h = self.gap(self.refine(feat)).flatten(1)                # convs now see 2x7x7, not 1x1x1
...
for fold, (tr, va) in enumerate(skf.split(pairs, LABELS), 1):
    ...
    sc = StandardScaler().fit(Xtr)                       # the one change kept
    ...
    print(f"  fold {fold} fusion best val acc: {best_acc:.2f}%   (paper fold target ~94-96%)",
          flush=True)
    pd.DataFrame(rows).to_csv(os.path.join(OUT, "paper_refine_folds.csv"), index=False)

df = pd.DataFrame(rows)
print("\n\n== PAPER PROTOCOL + per-clip norm + StandardScaler + refine head on a REAL volume ==")
...
df.to_csv(os.path.join(OUT, "paper_refine_folds.csv"), index=False)
